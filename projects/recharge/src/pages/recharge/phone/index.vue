<template>
  <view class="recharge-phone-page">
    <view class="recharge-phone">
      ...
      <recharge-prompt-group>
        <recharge-prompt-cell
          v-for="(value, index) in prompt.mobile_phone_credits"
          :key="index"
        >
          {{ index + 1 }}. {{ value }}
        </recharge-prompt-cell>
...
async function getBalance () {
  if (!data.verifyInfo.is_valid) return
  Taro.showLoading({
    title: '查询中',
    mask: true
  })
  const [res] = await queryBalance({
    ...
    calling_code: data.exampleInfo.calling_code
  })
  Taro.hideLoading()
  if (res.status === 200) {
